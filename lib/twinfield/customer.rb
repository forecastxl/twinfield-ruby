module Twinfield
  ...
  class Customer
    ...
    # Find all customers
    def all
      Twinfield::Process.new(@session).request(:process_xml_document, get_dimension_xml(@company, 'DEB')).body[:process_xml_document_response][:process_xml_document_result][:dimensions][:dimension]
    end

    # Find customer by twinfield customer code
    def find_by_code(code)
      #Twinfield::Finder.new(@session).search(Twinfield::FinderSearch.new('DIM', code, 0, 1, 0, { dimtype: 'DEB'} )).body[:search_response][:data]
      Twinfield::Process.new(@session).request(:process_xml_document, get_dimension_xml(@company, 'DEB', { code: code })).body[:process_xml_document_response][:process_xml_document_result][:dimension]
    end
